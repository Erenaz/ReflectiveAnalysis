...
#    det = generic_detector.GenericDetector(json_filename=f'configurations/station51.json', assume_inf=False, antenna_by_depth=False, default_station=51)
...
readCoREAS.begin(input_files, -spacing/2, spacing/2, -spacing/2, spacing/2, n_cores=cores, shape='radial', seed=None, log_level=logging.WARNING)	#use for direct triggers
...
for evt, iE, x, y in runCoREAS(CoREAS_mode):
    ...
    if(runid not in output):
        ...
        output[runid]['n_lpda_refl'] = 0
        output[runid]['n_lpda_dir'] = 0
        ...
        output[runid]['lpda_refl_mask'] = []
        output[runid]['lpda_dir_mask'] = []
        output[runid]['ant_zen'] = []
        output[runid]['dip_dir_SNR'] = []
        output[runid]['dip_refl_SNR'] = []
        output[runid]['lpda_dir_SNR'] = []
        output[runid]['lpda_refl_SNR'] = []
        ...
        # Trigger for direct dipoles
            ...
            if config == 'Stn51':
                ...
                simpleThreshold.run(evt, station, det,
                                            threshold = {key: value * 5/4.4 for key, value in lpda_thresh_high.items()},
                                            coinc_window = 32 * units.ns,
                                            triggered_channels=dir_LPDA_channels,
